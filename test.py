...
param_init = [1.6*d, 0.5*d, 0.75*10**(-41), 3*d, 0.5*10**(-42), 0.5]
...
def lnprior(param):
    ...
    if d < R_disk < 20*d and 0 < h_disk < d and 1e-43 < j_disk < 1e-39 and d < R_halo < 20*d and 1e-43 < j_halo < 1e-39 and T_bkg >= 0:
        return 0.0
...
def lnprob(param, nu, T_sky):
	lp = lnprior(param)
	if not np.isfinite(lp):
		return -np.inf
	print(lp)

	return lp + LP.diskhalobkg(param, nu, T_sky)
...
#sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, args=(1420e6, map_1420_dg), threads=8)

#SAMPLER = sampler.run_mcmc(init, 2000, progress=True)
